...
class Conn:
    def __init__(self, config, version, debug=False):
        ...
        try:
            ...
            print('', file=sys.stderr)
            print('>>> A database table for scan_lock does not exit. Creating...', file=sys.stderr)
            query = """
            CREATE TABLE "scan_lock" (
                    "server_id" INTEGER REFERENCES server PRIMARY KEY,
                    "locked_by_name" varchar(512),
                    "locked_by_host" varchar(512),
                    "locked_for"     int, -- scan_type.scheme
